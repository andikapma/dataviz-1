...
gdp_id %>% 
  filter(province %in% c("Banten", "DKI Jakarta", "Jawa Barat", "Jawa Tengah", "Di Yogyakarta", "Jawa Timur")) %>% # ekstrak provinsi di Pulau Jawa
  ggplot(aes(x = trade_hotel_and_restaurant, y = manufacturing, colour = province, size = ___)) +
  geom_point(alpha = 0.5)
```
...
gdp_id %>% 
  ...
  ggplot(aes(x = year, y = ___, fill = province)) +
  geom_area()
```
...
```{r}
ggplot(jabar_road, aes(x = district, y = road_length, fill = ___)) +
...
```{r}
highest_gdp %>% 
  ggplot(aes(x = ___, y = province, colour = year)) +
...
```{r}
highest_gdp %>% 
  ggplot(aes(x = gdp, y = ___(province, gdp), colour = year)) +
  geom_segment(aes(yend = province), xend = 0, colour = "grey50") +
  geom_point(size = 3)
  geom_label(aes(label = year), size = 2, hjust = -0.2)
```
...
```{r}
ggplot(___, aes(fill = electricity_total)) +
  geom_sf()
```
